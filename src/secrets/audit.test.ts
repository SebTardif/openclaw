...
describe("secrets audit", () => {
  ...
  it("does not flag custom-provider env SecretRef markers owned by config", async () => {
    ...
    await writeJsonFile(fixture.modelsPath, {
      providers: {
        ...
        [customProviderId]: {
          baseUrl: "https://factchat.example/v1",
          api: "openai-completions",
          // models.json serializes env SecretRefs as bare env-name markers.
          apiKey: customEnvId,
          models: [{ id: "grok-4.1-fast", name: "grok-4.1-fast" }],
        },
      },
  ...
  it("still flags bare custom markers when config does not own the same env SecretRef", async () => {
    const customProviderId = "custom-factchat-cloud-mindlogic-ai";
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          ...
          [customProviderId]: {
            baseUrl: "https://factchat.example/v1",
            api: "openai-completions",
            // Different env id than models.json marker — must stay plaintext.
  ...
  it("does not accept a config env SecretRef for a different provider id", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          "owned-provider": {
    ...
    await writeJsonFile(fixture.modelsPath, {
      providers: {
        // Marker matches owned-provider's env id but is under other-provider.
        "other-provider": {
          baseUrl: "https://other.example/v1",
          api: "openai-completions",
